<div class="modal fade" id="create-pilihan-ganda">
    <div class="modal-dialog  modal-fix" role="document">
        @if($assessments)
            <form action="{{ url('/admin/curriculum/assessment/'. $assessments[0]['id'] . '/subject/' . $subject['id'] . '/question/create') }}" method="POST" novalidate>
        @else
            <form action="{{ url('/admin/curriculum/assessment/0/subject/' . $subject['id'] . '/question/create') }}" method="POST" novalidate>
        @endif
            @csrf
