                ...
                <div class="col-md-10 col-lg-6">
                    ...
                    <div class="d-flex justify-content-between top-link align-items-center">
                        <div class="link-back d-flex align-items-center">
                            <a href="{{ url('/student/' . $stage['id'] . '/rounds') }}" class="text-link"><i class="kejar kejar-arrow-left"></i> <span>Kembali</span></a>
                        </div>
                    </div>
                    <!-- Breadcrumbs -->
                    <nav class="breadcrumb bg-transparent p-0">
                        <a class="breadcrumb-item" href="{{ url('/dashboard') }}">Beranda</a>
                        <a class="breadcrumb-item" href="{{ url('/student/' . str_replace(' ', '', $game) . '/stages/') }}">{{ $game }}</a>
                        <a class="breadcrumb-item" href="{{ url('/student/' . str_replace(' ', '', $game) . '/stages/' . $stage['id'] . '/rounds/') }}">{{ $stage['title'] }}</a>
